  ...
  <header class="site-header">
    <div class="site-header__inner">
      ...
      <nav id="storefront-navigation" class="site-nav" :class="{ 'site-nav--open': mobileMenuOpen }" aria-label="Navegación principal">
        <router-link to="/catalogo" :aria-current="route.path === '/catalogo' ? 'page' : undefined" @click="mobileMenuOpen = false">Catálogo</router-link>
        ...
        <router-link class="site-nav__cart" to="/carrito" :aria-label="`Carrito, ${cart.itemCount} artículos`" @click="mobileMenuOpen = false">
          ...
          <span>Carrito</span><b v-if="cart.itemCount" class="site-nav__cart-count">{{ cart.itemCount }}</b>
        </router-link>
      </nav>
    </div>
  </header>

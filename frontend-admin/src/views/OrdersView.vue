...
type Order = { _id: string; status: 'pending' | 'paid' | 'expired' | 'cancelled' | 'failed'; currency: string; items: ReceiptItem[]; total: number; customer: { name: string; email: string }; createdAt: string; paidAt?: string; receipt?: Receipt; receiptEmail?: { status: 'pending' | 'sent'; sentAt?: string; lastError?: string } };

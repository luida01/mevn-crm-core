...
Para probar el flujo visual, configura en el entorno del backend una clave de prueba `STRIPE_SECRET_KEY=rk_test_...` (restringida a Checkout Sessions) o `sk_test_...`, además de `STRIPE_WEBHOOK_SECRET=whsec_...`. En local, ejecuta `stripe listen --events checkout.session.completed,checkout.session.async_payment_succeeded,checkout.session.expired,checkout.session.async_payment_failed --forward-to localhost:5000/api/payments/webhook`; el CLI reenvía esos eventos y muestra el secreto `whsec_` para esa sesión. El checkout alojado por Stripe permite usar el número `4242 4242 4242 4242`, una fecha futura como `12/34`, cualquier CVC de tres dígitos y un código postal ficticio. Stripe simula la autorización; no hace falta una tarjeta bancaria real ni se realiza un cargo real. Consulta la [guía oficial de tarjetas de prueba de Stripe](https://docs.stripe.com/testing?numbers-or-method-or-token=tokens).
